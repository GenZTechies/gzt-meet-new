...
const New = () => {
  ...
  const handleShareURL = React.useCallback(async () => {
    await navigator.clipboard.writeText(meetingLink).then(() => {
      setCopied(true);
    });
  }, [meetingLink]);
  ...
  return (
    <main className="h-screen w-screen flex flex-col items-center justify-center bg-[url('/static/images/bg.svg')]">
      <div className="container flex items-center flex-col justify-center space-y-4 text-center">
        {loading ? (
          ...
          <>
            ...
            <Button
              className={`${
                copied ? "!bg-tertiary hover:bg-tertiary" : "bg-primary"
              }`}
